...
假设有AB两个事务，A先读取一个id不存在的数据，然后B插入这个id的数据并提交，然后A用update修改这个id的数据，然后A事务读取这个id，发现可以读到，发生了幻读。

# 讲一讲可重复读实现原理？（作业帮）
MVCC机制：每个事务在开始时创建一个快照，事务中所有读操作基于这个快照进行。

事务开始时会分配一个事务ID，read view是事务开始时创建的，包含m_ids（未提交事务ID列表），max_limit_id（生成read view时，系统应该分配给下一个事务的ID值），min_limit_id（当前未提交事务最小的事务ID）。如果数据版本的事务ID小于min_limit_id，说明该版本事务已提交，因此数据版本对当前事务可见。如果数据版本的事务ID大于或等于max_limit_id，说明该版本是在read view创建后新产生的，对当前事务不可见。如果数据版本事务ID结余min_limit_id和max_limit_id之间，需要检查该ID是否在m_ids中：如果在，且事务ID不等于creator_trx_id，则该版本对当前事务不可见。如果在，且事务ID等于creator_trx_id，则该版本对当前事务可见。如果不在m_ids中，说明事务在read view创建之前提交，该版本对当前事务可见。

锁机制：在可重复读级别下，事务对读取的数据加共享锁，防止其他事务修改。写操作会加排他锁，防止其他事务读取或修改。

# 联合索引使用情况判断。（百度，作业帮，哈啰）
explain create table myTest2 (a int, b int, c int, KEY suoyin(a, b, c));
explain select * from myTest2 where a=3 and b=5 and c=6;会命中联合索引
...
explain select * from myTest2 where a=3 and b>5 and c=6;由于b用到了范围查询，所以只用到了(a,b)联合索引，没有命中c的索引。
...
explain select * from myTest2 where a=3 and b=5 or b=4 and c=1;逻辑运算符AND优先级高于OR
...
普通字段，不是主键，没有索引，在可重复读隔离级别是加表锁，而在读已提交隔离级别时优化成加行锁。
...
如果查询条件用了主键或者索引，那么for update加行锁。

如果查询条件用了普通字段无索引，在可重复读隔离（RR）级别中是加表锁。

而在读已提交（RC）隔离级别中，for update仍然是加行锁，而不是表锁，是因为MySQL会走主键索引进行全表扫描过滤，每条记录都会加X锁，但是MySQL做了优化，会对扫描过程中不满足条件的记录进行解锁。
...
快照读通过读取数据的历史版本来避免加锁，适用于不修改数据的查询，允许数据在事务内保持一致性而不受并发事务的影响；

而当前读涉及对数据加锁（如S锁或X锁），确保读取的数据在事务执行期间不被其他事务修改或删除，用于需要修改数据的操作。
...
第三点就是设置的值是为了安全释放锁，释放的时候要检查key是否存在，还判断key对应的值是否和我指定的值一样。

# 设计分布式锁需要考虑什么问题？（得物，作业帮多次考）
互斥性：分布式锁基本要求，在redis或Zookeeper设置唯一的key实现。
...
实现方法：线程标识。获取锁时，除了设置锁的键值对，还可以将线程的唯一标识如线程ID存储在锁的值中，每次获取锁时，检查当前线程标识是否与锁的值匹配。
...
# redis内存溢出了，现在有少量bigkey，大量普通key，你怎么设计内存淘汰方案？（百度）
...
丢数据的可能性。
aof持久化。aof通过记录每次写操作保证数据持久性。然而如果在执行fsync系统调用前发生宕机，可能丢失最后一次fsync数据。

rdb持久化。通过定期快照保存数据。如果在快照生成过程中发生宕机，可能丢失上次快照以来的所有数据。

linux作为操作系统，本身是稳定的，如果是硬件故障如磁盘损坏或者操作系统级别的错误比如文件系统损坏，数据可能会丢失。
...
因为8个元素时红黑树是4层，超过后红黑树还是4层，而平均链表查找次数大于红黑树，所以在这个时候发生转化。

# hashmap为什么用红黑树不用avl树？为什么不用B+树（百度，腾讯）
# 为什么hashmap用红黑树不用跳表，不用B+树？（得物）
# 红黑树和平衡二叉树avl的区别？（58同城）
红黑树插入删除操作重新平衡操作相对简单，通常O 1次旋转，而AVL树可能要O logn次翻转。

红黑树是自平衡二叉搜索树，能在Log n时间内完成插入删除和查找，而B+树虽然在大规模数据存储和范围查询有优势，但在单个元素查找上，红黑树性能更好。且红黑树在Java标准库已经有成熟实现，而B+树没有。

红黑树实现相对简单，但跳表实现较复杂，在多线程环境下，跳表并发控制更为复杂。
...
占用64字节，需要考虑内存对齐。String对象本身16字节。char数组一个char2字节，总共20字节。数组对象本身占用20字节（对象头16字节+int长度字段4字节+padding3字节）。实际上应该总共60字节，还有4字节填充。

占用16字节。对象头12字节（8字节标记+4字节类指针），short2字节，boolean1字节，对齐填充1字节，总共16字节。
...
在每次执行哈希操作时，redis会检查是否要进行数据迁移，如果需要则迁移一部分数据，直到所有迁移完成。

为什么起一个线程：数据迁移频率可能过高，影响系统响应时间，redis可以选择在后台启动专门线程，集中进行数据迁移。
...
性能考虑：为提高性能，JVM可能会优先让新来的线程获得锁，而不是等待最久的线程，这种设计减少了上下文切换开销，但也牺牲了公平性。
...
使用synchronized可修饰实例方法，静态方法，或代码，分别锁定当前对象实例，当前类对象或指定对象。
...
**当持有锁的线程释放锁**，aqs会从队列中**唤醒一个等待的线程**，尝试让他获取锁。

**解锁过程**。当一个线程调用unlock方法时，**aqs的state字段会减1**。

如果state字段减到0，表示锁已经被完全释放，aqs会唤醒等待队列的下一个线程。
...
都不能被实例化。接口主要对类的行为做约束。抽象类用于代码复用，强调的是所属关系。

# 浅拷贝和深拷贝区别了解吗？什么是引用拷贝？
...
# 为什么重写equals方法必须重写hashCode方法？（得物）
...
指针碰撞，适用于堆内存规整的情况；空闲列表，适用于堆内存不规整情况。
...
DNS解析，TCP连接，发送HTTP请求，server处理请求，并返回HTTP报文，浏览器解析页面，连接结束。
...
# HTTPS如何保证自身秘钥安全性？（得物）
非对称加密：在SSL TLS握手阶段，服务器向客户端发送它的公钥，客户端用服务器公钥加密一个随机生成的对称密钥，并发给服务器。由于只有服务器私钥才能解密这个秘钥，因此保证了秘钥安全性。
...
避免资源浪费。如果只有两次握手，如果客户端syn报文在网络中阻塞，客户端没有收到ack报文，他就会重复发送syn，由于没有第三次握手，客户端不清楚客户端是否收到了自己回复的ack报文，所以服务端每收到一个syn只能先主动建立一个连接。
...
# TCP time_wait过多什么原因？怎么解决？（58同城，shopee）
...
代理的实现发生在**Spring容器初始化阶段**之前。当Spring容器启动时，会扫描所有的bean，并根据配置决定是否要为这些bean创建代理对象。如果一个bean需要被aop增强，spring会在bean初始化完成后，为他创建一个代理对象。
...
工厂设计模式（通过 BeanFactory 和 ApplicationContext 创建 bean），代理设计模式（用于 Spring AOP 的实现），单例设计模式（默认的 Bean 作用域），模板方法模式（如 JdbcTemplate 和 HibernateTemplate），包装器设计模式（用于动态数据源切换），观察者模式（Spring 事件驱动模型）和适配器模式（用于 Spring AOP 和适配 Controller）。
...
在嵌套事务中，事务上下文会被传递。方法a开启事务后，事务上下文会被传递给方法b和c。在spring中，事务上下文通过threadlocal传递。
