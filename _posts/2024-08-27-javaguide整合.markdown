...
## MySQL输入select指令后会发生什么？（MySQL查询语句发生了什么）（美团，字节）
...
## mysql执行更新语句发生了什么（undolog和binlog写的顺序？）（字节）
sql解析。语法检查。

查询优化。根据索引生成查询计划。

执行阶段。1.加锁。2.行定位，根据where定位要更新的行，加载到缓冲池。3.生成undolog。更新前，生成undolog。4.生成redolog。5.更新。写入数据页。

提交。执行Commit所有更新才会生效，binlog记录这次更新。
...
## t0：事务1 select value from tb where id =1; t1：事务3 update tb set value=value +1 where id=1; t2：事务3提交。t3：事务2 update tb set value=value+1 where id=1; t4：事务2 select value from tb where id=1;t5：事务2提交。t6：事务1 select value from tb where id =1;t7： 事务1提交。问事务1，事务2读出的value是多少？（美团）
...
## select for update会不会加表锁，什么情况下加表锁？（腾讯，度小满，百度，淘天）
没有索引，查询需全表扫描时，加表锁。
...
## lru（LRU，最近最少使用）和lfu（LFU，最近最不经常用）的原理，lru和lfu可以有什么优化（腾讯）
...
## redis集群模式下如果节点宕机，如何进行的收缩和扩容（字节）
...
## 针对redis的热数据你会怎么处理？（针对redis的热key你会怎么处理？）（热key的危害及解决方法）（字节）
...
## redis的RDB持久化指令BGSAVE操作是fork一个子进程进行持久化的，为什么不创建一个子线程完成持久化？（字节）
...
## hashmap为什么用红黑树不用avl树？为什么不用B+树，为什么不用跳表？（百度，腾讯）
...
## HashMap的长度为什么是2的幂次方？（或者讲讲HashMap的扩容机制吧，饿了么考过）（度小满，美团）
...
## 为什么TCP4次挥手之后要等待2MSL，2这个倍数是怎么确定的，为什么不是3，不是1.5等（腾讯，字节）
...
## TCP close_wait过多什么原因？（字节，腾讯多次考，拼多多）
服务端没有及时调用close关闭套接字。
...
## nginx和springcloud gateway的区别（腾讯）
