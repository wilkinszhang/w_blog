...
## MySQL有哪些常见的索引？（讲讲索引）（讲讲数据库索引）（讲讲MySQL索引）（介绍一下 mysql 的索引吧）（腾讯，携程）page 47
...
## MySQL输入select指令后会发生什么？（MySQL查询语句发生了什么）（请描述一条SQL查询语句在MySQL中是如何执行的？）（美团，字节，高德）page 43
连接权限验证。
...
## 针对redis的热数据你会怎么处理？（针对redis的热key你会怎么处理？）（热key的危害及解决方法）（字节）page 147 page 34
...
## Linux命令了解吗？（讲讲linux常用5个命令）（腾讯，美团）page 1
...
## Spring框架用了哪些设计模式？（科大考过，58同城）page 155
