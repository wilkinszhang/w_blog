...
## 如何避免数据库死锁？（网易雷火）page 44
死锁原因：事务操作数据时持有互斥锁，并在请求其他资源形成循环等待。如innodb，rr隔离级别下，用非唯一索引select for update，触发gap lock、next key lock，但执行查询sql获取的gap lock不会阻塞，当执行插入sql时，会在间隙获取插入意向锁，插入意向锁与gap lock是冲突的。这时可能会形成死锁。
...
## MySQL默认隔离级别，可重复读，什么情况不能解决幻读问题？（作业帮）page 47
...
## EXPLAIN SELECT  * FROM example_table WHERE b = 1 AND c > 2 AND a = 3;（快手，淘天）
范围扫描range。**c命中，索引下推。**
...
## 慢查询explain分析的时候，type和extra出现什么内容表示不太优？（EXPLAIN中的哪些type和extra值表明查询可能存在性能问题？）（美团）page 1
...
## MySQL有哪些常见的索引？（讲讲索引）（讲讲数据库索引）（讲讲MySQL索引）（腾讯，携程）page 1
索引是一种数据结构，可以让数据库查询不必全表扫描。

基本原理。1.哈希表。用哈希函数将key映射到固定为止，适用等值查询。2.有序数组。用二分查找，适用范围查询，但插入删除成本高。3.B+树，在关系型数据库用的是B+树和B树。B+树中叶子节点存储数据或指向数据行。
...
## MySQL输入select指令后会发生什么？（MySQL查询语句发生了什么）（请描述一条SQL查询语句在MySQL中是如何执行的？）（美团，字节）page 43
连接权限验证。
...
## MySQL binlog和redolog写顺序（字节）page 42
...
## select for update会不会加表锁，什么情况下加表锁？（腾讯，度小满，百度，淘天）page 96
没有索引，查询需全表扫描时，加表锁。
...
## 如果A是主键，查询语句是"A=0"，此时加的是record锁还是next-key锁？如果A不是主键而是非唯一索引呢？（得物）page 97
...
## select * from user where userid=5 for update，假如userid是索引但是没有5这个数据，锁的是什么？假如没有索引也没有5这个数据，锁的是什么？（在执行SELECT * FROM user WHERE userid=5 FOR UPDATE;时，如果满足条件的记录不存在，那么：如果 userid 是索引，锁定的是什么？如果没有索引，锁定的又是什么？）（快手）page 97
...
## 如果A是非唯一索引，表里只有"A=1"和"A=10"两条数据，此时有两条命令："SELECT * FROM ... WHERE A=5 FOR UPDATE"；"SELECT * FROM ... WHERE A=6 FOR UPDATE"。请问这两条命令会互斥吗，什么时候互斥、什么时候不互斥？（得物）page 97
...
## 解决并发问题，数据库和分布式锁哪个效率好一点？（拼多多）page 97
...
## 雪花算法由符号位，时间戳，机器号和序号组成，怎么保证全局有序性？时间戳只会增大不会减小，序号可以保证一毫秒内的有序性。那你同一毫秒内机器号大的生成序号小，最后总大小反而大怎么办？（雪花算法如何通过时间戳、机器号和序号来保证生成的全局唯一、有序的ID？假设在同一毫秒内，不同机器生成的ID中，机器号大的节点可能得到较小的序号，导致整体ID数值更大，这种情况如何确保全局顺序不出错？）（字节）page 110
...
## 向 1G 的 Redis 陆续写入 2G 的数据、会发生什么？（猿辅导）page 98
...
## lru（LRU，最近最少使用）和lfu（LFU，最近最不经常用）的原理，lru和lfu可以有什么优化（腾讯）page 110
...
## redis持久化有多少种丢数据的可能？linux会丢数据吗？（携程）page 98
...
## 针对redis的热数据你会怎么处理？（针对redis的热key你会怎么处理？）（热key的危害及解决方法）（字节）page 98 page 34
热key是访问频率很高的key。有两种情况。1.在高并发场景如秒杀、热搜。2.资源分配不平衡导致访问热点。

解决方案。1.把热key拆分，实现流量分流。2.用多级缓存设计，增加本地缓存减少目标节点访问。3.对访问频率高的节点扩容，用负载均衡方式分散流量。
...
## 布隆过滤器原理介绍一下。（得物，58同城，腾讯，TME）page 34
...
## redis内存溢出了，现在有少量bigkey，大量普通key，你怎么**设计**内存淘汰方案？（百度）page 99
...
## redis有一个bigkey你怎么删掉？（shopee）page 99
...
## hashmap为什么用红黑树不用avl树？为什么不用B+树，为什么不用跳表？（百度，腾讯）page 99
...
## redis zset listpack和跳表的区别？什么时候用哪一种？redis为什么用跳表不用红黑树？（阿里）page 74
...
## String str1=“abc”和String str2=new String("abc")他们是一样的吗? （Java 为什么直接赋值的字符串（例如 "abc"）和用 new 创建的字符串（new String("abc")）不是同一个对象？）（滴滴考过，百度）page 99
...
## 如何设计一个动态线程池，支持运行时修改参数？（快手）page 100
参考。
...
## 线程池核心线程数量怎么确定？（百度）page 50
## 线程池参数考量 有没有一个既定的公式（字节）
IO密集型：线程数配置为核心数*2。
...
调优。根据上述理论计算出一个初步线程数，然后经过压测验证。用线程平均**执行**时间和线程平均**整体**时间衡量性能，当线程数增加时，线程平均**执行**时间会逐渐增加，而线程平均**整体**时间则先减少后增加，找到这个拐点就是最佳核心线程数量。


<!-- 任务特性：CPU密集型或IO密集型。
系统资源：CPU核心数，内存容量。 -->

## 2000qps，响应时间100ms，四核，I/O密集型，怎么设置核心线程数（网易）page 109
...
## 线程池有多个线程竞争同一个资源怎么办？没抢到资源的线程处于什么状态？（腾讯）page 100
同步机制：
...
## 为什么用线程池不用进程池？多线程能否完全取代多进程？（腾讯） page 117
## 在多核CPU可以让多个进程并发的场景下，为什么还需要线程？（滴滴）page 100
...
## HashMap的原理？以及不同情况下的复杂度？（字节，小鹏考过，58同城，快手）page 101
...
## hashmap源码中，计算hash值为什么有一个高16位和低16位异或的过程？（百度考过，蚂蚁）page 101
...
## concurrenthashmap怎么实现线程安全的？（快手多次考，美团）
...
## concurrenthashmap在JDK1.7中size怎么计算的？和JDK1.8有什么区别？（concurrenthashmap size方法的实现？）（滴滴多次考）
...
## treeset可以存null值吗，为什么？Concurrenthashmap可以存null吗，为什么？HashMap为什么可以存入null值（阿里）page 101
## ConcurrentHashMap的value可以为null吗？为什么hashmap的可以（网易）
...
## 讲一讲volatile底层原理？（得物多次考，shopee，哈啰，美团）
实现依赖于内存模型和内存屏障。
...
## 讲一讲synchronized关键字的底层原理（重要）（得物，字节）page 46
...
## synchronized和Reentrantlock reentrantlock的区别（重要）（得物，快手，美团）
## Java中的锁有了解吗？（58同城）
...
## reentrantlock的可重入的原理是什么？（京东，腾讯）page 58
...
## reentrantlock公平和非公平的实现原理是什么？（京东，阿里，快手）page 102
...
## AQS队列为空时线程加入队列发生什么？（百度）page 102
参考。
...
## synchronized和Reentrantlock哪个性能更好？（小红书）page 102
...
## 为什么重写equals方法必须重写hashCode方法？（得物，美团）page 103
...
## 解释JVM中Minor GC（小型垃圾收集）、Major GC（主要垃圾收集）和Full GC（完全垃圾收集）之间的区别。是什么触发了每种类型的垃圾收集？（滴滴考过）page 103
...
数据库性能调优。1.避免死锁。（1）优化SQL。用慢查询日志和explain定位瓶颈，针对具体场景，设计和调整索引，如经常更新的记录用主键更新，避免用辅助索引更新，减少死锁风险。（2）事务和锁管理。长事务容易死锁，拆解长事务，针对死锁，设置合理的锁等待超时参数innodb lock wait timeout。写事务代码时，保证操作遵循固定顺序。（3）隔离级别。对允许脏读幻读场景，用读已提交减少间隙锁，2.索引优化。（1）对主键索引用自增字段，保证数据顺序写入，减少页分裂和内存碎片。对于辅助索引要考虑能否索引覆盖，避免回表。注意联合索引顺序。（2）索引失效。重写SQL或使用hint比如force index强制使用索引。

## 做过MySQL调优吗？（得物，作业帮多次考）page 11
...
## 为什么TCP4次挥手之后要等待2MSL，2这个倍数是怎么确定的，为什么不是3，不是1.5等（tcp为什么四次挥手后等待2msl，而不是3msl，1.5msl？）（腾讯，字节）page 44
确保最终ACK可靠性。主动关闭一方发送最后ACK后，如果该ACK丢失，被动的一方重传FIN。处于Time wait状态的一方必须能正确应对这种重传。

防止旧报文干扰新连接。2MSL保证所有属于该连接的报文都已超时被丢弃。一个MSL保证接收方收到最后ACK，另一个MSL保证网络中延迟数据包已过期。
...
## 我服务挂了，重启显示端口占用，啥情况，排查一下（腾讯）
进程未退出。服务异常退出，仍存在僵尸进程或孤儿进程，用lsof -i查看是否有该进程运行。

## 僵尸进程怎么解决？（腾讯）
僵尸进程是子进程已终止，父进程未调用wait waitpid获取子进程退出状态，子进程PCB仍保留在系统进程表。
...
## 为什么TCP三次握手？而不是两次或四次？（滴滴考过，58同城，腾讯，叮咚买菜）page 105
## 讲一下tcp四次挥手？（作业帮）
...
## TCP close_wait过多什么原因？（字节，腾讯多次考，拼多多）page 116
服务端没有及时调用close关闭套接字。
...
## TCP time_wait过多什么原因？怎么解决？（58同城，shopee，腾讯多次考）page 105
...
## 短连接和长连接在防火墙场景下用哪个比较好？（腾讯）page 116
...
## TCP time_wait发生在四次挥手的哪个阶段？（腾讯）page 105
客户端第四次挥手阶段。
...
## Bean的生命周期了解吗？（小红书，得物，京东）page 106
## Spring生命周期了解吗？
...
## 谈谈自己对AOP的理解。（滴滴，快手考过）（Spring AOP 的原理，以及它的代理是在哪个阶段实现的）page 116
## AOP发生在bean的生命周期的哪个阶段？（百度）page 107
...
## 如果构造函数内存在循环依赖还能解决循环依赖吗？（shopee）page 109
...
## Springboot核心原理是什么？Springboot容器的key是什么？value是什么？可以用泛型吗？（腾讯）page 108
...
## spring怎么知道所有bean创建完的？（快手）page 108
...
## beanFactory和factoryBean的区别（阿里）page 108
...
## Spring框架用了哪些设计模式？（科大考过，58同城）page 107
...
## Spring事务什么时候失效？事务失效？（百度，得物多次考，哈啰，网易）page 107
...
## Spring事务的原理是什么？（得物，58同城）page 107
基于AOP代理模式实现的。
...
## SpringBoot的自动配置如何实现的？什么是SpringBoot自动装配？（腾讯，恒生）page 107
...
## nginx和springcloud gateway的区别（腾讯）page 52
