...
# 500个人去抢一瓶茅台，两个服务器，一台服务器负载300，数据库负载10，设计一个秒杀系统。（得物）
...
# redis主节点挂掉了，哨兵没有选出下一个节点，这时候写的数据会怎么样？（百度）
参考。

所有写操作无法完成，因为redis无法讲这些写操作同步到从节点或新的主节点。

预防措施：配置redis持久化机制，主节点尽快从持久化文件恢复数据。
...
# 实现一个栈的自定义函数increase，从栈底每个元素增加一个给定值，怎么优化？（百度）
...
# 数据库表删除了10万条数据，但是表的磁盘占用没变化是为什么？（百度）
...
# 一张student表，其中字段id,age,name，其中id,age有索引，select * from student where age > 18，这个语句是怎么走索引的？（得物）
索引扫描：用age的索引进行范围扫描range scan，从索引中找到第一个大于18的值，并继续扫描。
...
# 如果A线程插入一条记录，但是未提交事务，B线程执行select for update会怎么样？（得物）
如果事务A插入了记录但未提交，这条记录会被事务A持有排他锁。事务B执行select for update会被阻塞，直到事务A提交或回滚。

# 数据库有两个事务要修改数据库同一行数据一个字段，一个改为A，一个B。一个事务到达，开启了事务但没有commit，这时另一个事务到达，请问此时B事务会怎样。继续发问：在读已提交级别会怎样？上面两个事务的修改结果是啥，都有哪些可能结果？（腾讯）
B事务会被阻塞，在读已提交和可重复读都被阻塞。

如果A事务成功提交，且B事务也提交，则字段先改为A再改为B。
如果A事务回滚，且B事务提交，则字段修改为A被撤销，恢复为initial，再被修改为B。
...
内存使用率。如果a服务在等待b服务响应的过程中需要缓存更多的请求或数据，内存使用率可能增加。
...
# 讲一讲MySQL 服务器怎么分析连接情况？有一大堆连接怎么处理？（shopee）
...
# 设计抢红包功能，要求金额随机，先来先领，不能超发。（shopee）
...
# 实现一个抢红包的算法（游卡）
随机分配：额度在0.01到剩余平均值\*2之间，(remaining_amount/remaining_packet)*2，剩余金额/剩余人数\*2。
...
# 大文件小内存数据排序问题，两两归并比较慢怎么优化？多路归并的时间复杂度是多少？（腾讯）
用多路归并排序，是一种外部排序的实践。
假设k个块，每个块包含n除以k个元素。
  多路归并流程
...
# 设置了JVM Xmx为1G，但实际Java进程占用了1.3G，你觉得多出来的0.3G来自哪个部分？（百度）
...
# 1000个人中有一位新冠患者，如何用最少的试管和检测次数，找到这个患者？（十个小白鼠，1000瓶水，怎么试出来哪瓶水有毒？）（京东，shopee）
...
# 给定ab两个文件，各存放50亿个url，每个url占64字节，找出ab两个文件共同的url，内存限制是4G字节。（大文件diff）（度小满，百度，shopee）
...
# 给定100亿个无符号乱序整数序列，如何求出100亿个数的中位数，内存只有512M字节。（腾讯）
分治。
...
# 假设有1000万个查询串，每个串的长度不超过255字节，去重后有300万个，请统计**最热门**的10个查询串，要求使用的内存不超过1G字节。     
...
# 40亿只鸟，每个鸟都有自己的id，经过一个信号塔会记录，信号塔容量为1G字节，晚上工作人员会带着id查询这个鸟今天是否经过，怎么设计？如果鸟更多呢？400亿呢？（腾讯）
