...
缓存机制，redis缓存将热点数据缓存到redis中，减少对数据库直接访问。本地缓存，在应用层引入guavacache，减少对redis的访问。
客户端缓存，减少请求服务端。
...
keepaliveTime应设置大于平均延时，确保连接在大多数情况下不被过早关闭，同时也应该小于最大延时，避免连接在极端情况下长时间保持，占用资源。可以选择折中的4毫秒。（不是很确定）
...
首先我们可以通过mysql命令来分析当前的连接情况，用show processlist命令查看当前所有连接和状态。
...
# 你觉得安卓垃圾回收用的什么策略（小红书）
用的分代垃圾回收策略，对象分为新生代和老年代，新生代用复制算法，老年代用标记清除整理算法，减少停顿时间。

# 假设有一个10万的数据请求，你会有什么方法来实现这些数据的增删改查？

数据分片。将数据分散到多个数据库实例或表中，这样可以**提高并发处理能力**，比如可以根据用户id哈希值决定数据存储的位置。
...
数据去重。可以用**哈希表**实现去重。哈希表hashmap每个条目占用大约20字节，300万个条目大约60M字节，在1G字节内存限制可以接受。

统计频率。统计每个查询串出现频率， 可以继续使用哈希表，这个步骤不会增加额外内存开销。

找出最热门的查询串。使用**最小堆**维护当前频率最高的10个查询串，内存开销是常数级的。
