...
# go语言interface底层如何实现（腾讯）page 115
...
# go语言中Channel底层的实现（Channel底层原理）（腾讯）page 116
# channel 是怎么保证线程安全？
...
# Go语言channel关闭了，读写这个channel会发生什么（腾讯）page 115
...
# go语言结构体作为参数传递时是传值还是传指针（字节）page 114
结构体是值传递。

# go语言如何对map排序（字节）page 114
...
# go语言 cap和len区别（百度）page 112
cap返回切片容量，len返回切片实际长度。

# go语言能往已经closed的Channel写数据吗（百度）page 112
会导致panic。
...
# 对于go语言 sync.map底层如何实现并发安全？（百度）page 111
...
# go语言map并发访问时panic如何实现的（小米）page 111
...
# golang怎么查找性能瓶颈？（golang gc如何调优）（golang 垃圾回收怎么调优）（字节）page 53
...
# go中如何将string转为切片[]byte，不申请额外空间（得物）page 43
